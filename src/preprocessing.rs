...
#[cfg(test)]
mod tests {
    use super::{type_of_file,get_word_counts_for_train_file};
    ...
    fn get_word_counts_for_test_file_test(){
        let vec1 = vec![(String::from("a"),2 as usize),(String::from("bird"),1 as usize),(String::from("man"),1 as usize)];
        let vec2 = vec![(String::from("dog"),1 as usize),(String::from("m"),1 as usize)];
        assert_eq!(get_word_counts_for_train_file("A bird, a man",false),vec1);
        assert_eq!(get_word_counts_for_train_file("A a a a a a is dog I'm",true),vec2);
    }
